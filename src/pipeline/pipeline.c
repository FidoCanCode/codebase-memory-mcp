...
static int run_extraction_phase(cbm_pipeline_t *p, cbm_pipeline_ctx_t *ctx,
    ...
    cbm_pipeline_set_rocq_projmap(cbm_rocq_projmap_build_from_repo(ctx->repo_path));
    // Then the cross-file notation seed (uses the load-path map above).
    cbm_rocq_set_seeddb(cbm_rocq_seeddb_build_from_repo(files, file_count));
...
cleanup:
    ...
    cbm_pipeline_set_pkgmap(NULL);
    {
        ...
        rocq_seeddb_free((RocqSeedDB *)cbm_rocq_get_seeddb());
        cbm_rocq_set_seeddb(NULL);
    }
    cbm_discover_free(files, file_count);
